...
if(!empty($aid) && $uid!=0 && !empty($uid)){
	...
	$sql = "INSERT INTO `Release_Feed`.`afavorite` (`aid`, `uid`, `prcid`,`flg`) VALUES ('".$aid."', '".$uid."', '".$prcid."', '1');";
	if(mysql_db_query($db_name, $sql)){//afavoriteにinsert出来る場合
		$sql = "UPDATE `Release_Feed`.`article` SET `favorite` = `favorite` + '1' WHERE `aid` = '". $aid ."' AND `prcid` = '". $prcid ."';";
		...
		if($flg == 1){//flg=1
		 	...
		 	$sql = "UPDATE `Release_Feed`.`article` SET `favorite` = `favorite` - 1 WHERE `aid` = '". $aid ."';";
			if(mysql_db_query($db_name, $sql)){
			 	$sql = "UPDATE `Release_Feed`.`afavorite` SET `flg`= '0' WHERE `aid` = '". $aid ."' AND `uid` = '". $uid ."';";
			 	if(mysql_db_query($db_name, $sql)){
			...
			if(mysql_db_query($db_name, $sql)){//articleに+1できた
				$sql = "UPDATE `Release_Feed`.`afavorite` SET `flg`= '1' WHERE `aid` = '". $aid ."' AND `uid` = '". $uid ."';";
				if(mysql_db_query($db_name, $sql)){
					$data = array('status' => "success");
			 		echo json_encode($data);
			 	}else{
